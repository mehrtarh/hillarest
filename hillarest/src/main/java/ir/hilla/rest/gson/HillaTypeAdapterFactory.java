 ...
 * model: <pre>   {@code
 *
 *   public class LowercaseEnumTypeAdapterFactory implements HillaTypeAdapterFactory {
 ...
 *         }
 *
 *         public T read(HillaJsonReader reader) throws IOException {
 ...
 * HillaGsonBuilder} for them to take effect: <pre>   {@code
 *
 ...
 * multiset's type token. A {@code HillaGson} is passed in to {@code create} for
 * just this purpose: <pre>   {@code
 *
 *   public class MultisetTypeAdapterFactory implements HillaTypeAdapterFactory {
 ...
 *       HillaTypeAdapter<?> elementAdapter = gson.getAdapter(HillaTypeToken.get(elementType));
 *       return (HillaTypeAdapter<T>) newMultisetAdapter(elementAdapter);
 *     }
 *
 *     private <E> HillaTypeAdapter<Multiset<E>> newMultisetAdapter(
 *         final HillaTypeAdapter<E> elementAdapter) {
 *       return new HillaTypeAdapter<Multiset<E>>() {
 *         public void write(HillaJsonWriter out, Multiset<E> value) throws IOException {
 *           if (value == null) {
 *             out.nullValue();
 *             return;
 *           }
 ...
 *         }
 *
 *         public Multiset<E> read(HillaJsonReader in) throws IOException {
 ...
 * the foundation of how HillaGson converts Java objects and collections. Whenever
...
public interface HillaTypeAdapterFactory {
   ...
   * support {@code type}.
   */
  <T> HillaTypeAdapter<T> create(HillaGson gson, HillaTypeToken<T> type);
}
