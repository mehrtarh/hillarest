 ...
 */
public final class HillaStreams {
  private HillaStreams() {
    throw new UnsupportedOperationException();
  }

  /**
   * Takes a reader in any state and returns the next value as a HillaJsonElement.
   */
  public static HillaJsonElement parse(HillaJsonReader reader) throws HillaJsonParseException {
    boolean isEmpty = true;
    try {
      reader.peek();
      isEmpty = false;
      return HillaTypeAdapters.JSON_ELEMENT.read(reader);
    } catch (EOFException e) {
      /*
       * For compatibility with JSON 1.5 and earlier, we return a HillaJsonNull for
   ...
   */
  public static void write(HillaJsonElement element, HillaJsonWriter writer) throws IOException {
    HillaTypeAdapters.JSON_ELEMENT.write(writer, element);
  }
